...
class ApbImageWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if(audio.imagePath != null) {
      return AipImageWidget(image: AipFileImage(path: audio.imagePath!), height: size, width: size, fit: BoxFit.contain,);
    }
    else if(audio.imageUrl != null) {
      return AipImageWidget(image: AipUrlImage(url: audio.imageUrl!), height: size, width: size, fit: BoxFit.contain);
    }
    else {
...
class OverflowBlurWidget extends StatelessWidget {
  const OverflowBlurWidget({
    super.key,
    required this.child,
    required this.height,
    this.blurHeight = 30.0,
...
/// A variant that automatically detects theme brightness
class ThemeAwareOverflowBlurWidget extends StatelessWidget {
  const ThemeAwareOverflowBlurWidget({
    super.key,
    required this.child,
    required this.height,
    this.blurHeight = 30.0,
